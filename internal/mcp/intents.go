...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("create_an_http_monitor",
			mcplib.WithDescription("Preview the create-monitor request body before sending it; drop --dry-run to apply."),
			mcplib.WithBoolean("dry_run", mcplib.Description("Override the recipe's --dry-run value.")),
		),
		handleCreateAnHttpMonitor,
	)
}

// handleCreateAnHttpMonitor runs the create_an_http_monitor recipe intent: Preview the create-monitor request body before sending it; drop --dry-run to apply.
func handleCreateAnHttpMonitor(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
