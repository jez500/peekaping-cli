...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "peekaping-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("PEEKAPING_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("PEEKAPING_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "peekaping-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/peekaping-pp-cli/feedback.jsonl.
When ` + "`PEEKAPING_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`PEEKAPING_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  peekaping-pp-cli feedback list
  peekaping-pp-cli feedback list --limit 5
  peekaping-pp-cli feedback list --json`,
