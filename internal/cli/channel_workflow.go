...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  peekaping-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  peekaping-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"api-keys", "api-keys-config", "health", "maintenances", "monitors", "notification-channels", "peekaping-version", "proxies", "status-pages", "tags"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  peekaping-pp-cli workflow status

  # Show status as JSON
  peekaping-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("peekaping-pp-cli")
			}
