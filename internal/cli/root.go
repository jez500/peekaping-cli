...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "peekaping-pp-cli",
		Short: `Peekaping CLI — The first CLI for Peekaping: full monitor CRUD plus offline down/flapping/uptime analytics no single API call can answe…`,
		Long: `Peekaping CLI — The first CLI for Peekaping: full monitor CRUD plus offline down/flapping/uptime analytics no single API call can answe…
...
Health check: run 'peekaping-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("peekaping-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'peekaping-pp-cli profile list')")
	...
	rootCmd.AddCommand(newBadgeCmd(flags))
	rootCmd.AddCommand(newMaintenancesCmd(flags))
	rootCmd.AddCommand(newMonitorsCmd(flags))
	rootCmd.AddCommand(newNotificationChannelsCmd(flags))
	rootCmd.AddCommand(newPeekapingAuthCmd(flags))
	rootCmd.AddCommand(newProxiesCmd(flags))
	...
	rootCmd.AddCommand(newNovelCertsCmd(flags))
	rootCmd.AddCommand(newNovelDownCmd(flags))
	rootCmd.AddCommand(newNovelFlappingCmd(flags))
	rootCmd.AddCommand(newNovelIncidentsCmd(flags))
	rootCmd.AddCommand(newNovelTagHealthCmd(flags))
	rootCmd.AddCommand(newNovelWorstUptimeCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newHealthPromotedCmd(flags))
	rootCmd.AddCommand(newPeekapingVersionPromotedCmd(flags))
