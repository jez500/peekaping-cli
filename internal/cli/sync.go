...
Resource scoping:
  ...
  peekaping-pp-cli sync

  # Sync specific resources only
  peekaping-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  peekaping-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  peekaping-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  peekaping-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  peekaping-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/peekaping-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "cert_exp":
		return db.UpsertCertExp(data)
	case "ping":
		return db.UpsertPing(data)
	case "response":
		return db.UpsertResponse(data)
	case "status":
		return db.UpsertStatus(data)
	case "uptime":
		return db.UpsertUptime(data)
	case "pause":
		return db.UpsertPause(data)
	case "resume":
		return db.UpsertResume(data)
	case "heartbeats":
		return db.UpsertHeartbeats(data)
	case "reset":
		return db.UpsertReset(data)
	case "stats":
		return db.UpsertStats(data)
	case "tls":
		return db.UpsertTls(data)
	default:
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"api-keys":              "/api-keys",
		"api-keys-config":       "/api-keys/config",
		"health":                "/health",
		"maintenances":          "/maintenances",
		"monitors":              "/monitors",
		"notification-channels": "/notification-channels",
		"peekaping-version":     "/version",
		"proxies":               "/proxies",
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "heartbeats", ParentTable: "monitors", ParentIDParam: "id", PathTemplate: "/monitors/{id}/heartbeats", KeyField: "", PathParams: []dependentPathParamDef{
			{Param: "id", Field: "id"},
		}},
	}
}
