...
type Config struct {
	...
	Path            string            `toml:"-"`
	PeekapingApiKey string            `toml:"api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("PEEKAPING_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "peekaping-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("PEEKAPING_API_KEY"); v != "" {
		cfg.PeekapingApiKey = v
		cfg.AuthSource = "env:PEEKAPING_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.PeekapingApiKey != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("PEEKAPING_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	token := c.PeekapingApiKey
	if token == "" {
		return ""
	}
	if c.PeekapingApiKey == "" {
		return ""
	}
	return token
}
