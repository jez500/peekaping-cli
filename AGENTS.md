# Peekaping Printed CLI Agent Guide

This directory is a generated `peekaping-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
peekaping-pp-cli doctor --json
peekaping-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
peekaping-pp-cli which "<capability>" --json
peekaping-pp-cli <command> --help
```
...
```bash
peekaping-pp-cli <command> --agent
```
...
```bash
peekaping-pp-cli <command> --help
peekaping-pp-cli <command> --dry-run --agent
```
